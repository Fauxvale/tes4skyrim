...
from progress import track, track_records
...
def import_plugin(export_dir: str, output_path: str, masters: list = None,
    ...
    for sig, target_sig, rec in track_records(work_items, len(by_type.get('LAND', [])), len(by_type.get('PGRD', []))):
...
def _precompute_land(by_type: dict, export_dir: str) -> dict:
    ...
    with ProcessPoolExecutor(max_workers=workers,
        ...
        for results in track('Landscape', ex.map(convert_worker.convert_chunk, chunks), chunks, len):
...
def _precompute_navmeshes(by_type: dict, writer: PluginWriter,
    ...
    if len(jobs) == 1 or n_workers == 1:
        ...
        for job in track('Navmesh', jobs, jobs):
        ...
        with ProcessPoolExecutor(
            ...
            for key, result in track('Navmesh', ex.map(navm_worker.run_job, jobs, **_map_kwargs), jobs):
                cache[key] = result
