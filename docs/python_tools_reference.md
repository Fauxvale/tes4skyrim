...
- **Collision winding**: `python tools/collision_winding.py <nif_or_dir> [--converted] [--floor-orientation] [--top N] [--workers N]` — finds inverted floor halves (the "fall through half the floor" bug). A vanilla Oblivion tree should report only a handful of hits. **The default scan only detects a MIXED up/down edge pair, so a uniformly reversed floor reports zero** (Morrowind_ob's `inuhlaaluuroomuside.nif` scores 0 while being unwalkable) — use `--floor-orientation` for that class, which reports whether each mesh's lowest near-horizontal surface faces up (walkable) or down (fall-through).
...
- **★ CheckNavMesh on generated cells**: `python tools/navmesh_cell_check.py <EditorID> [...]` — same rules against fresh `build_navmesh` output, no ESM needed. Seconds per cell, so generator fixes iterate directly.
