...
- `tools/tes4_nif_analyzer.py` handles BOTH versions (PyFFI dispatches on version); the `tes5_` re-export shim was removed 2026-08-25
...
- Debug single tiles without a full run: `python tools/terrain_lod_render.py` (rebuilds specific tiles in-process, reports water quads, dumps diffuse PNG). `python -m tools.terrain_lod_tex_probe [--cell X Y]` audits LTEX→TXST→dds resolution and per-cell layers.
