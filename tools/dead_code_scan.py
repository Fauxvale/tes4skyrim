...
Usage::
    ...
    python tools/dead_code_scan.py                      # whole repo
    python tools/dead_code_scan.py --path tes5_import   # report one package
    python tools/dead_code_scan.py --kind function      # only functions
    python tools/dead_code_scan.py --min-lines 15       # only large bodies
    python tools/dead_code_scan.py --modules            # unimported modules
    python tools/dead_code_scan.py --json temp/dead.json
"""
...
def main() -> int:
    ...
    for d in all_defs:
        ...
        if keepers:
            # A tools/ helper called by tools/ is a normal intra-tier call, not
            # a finding -- only cross-tier life support is.
            if own_tier != "pipeline" and keepers == [own_tier]:
                continue
