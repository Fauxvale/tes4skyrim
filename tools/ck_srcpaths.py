...
Usage:
    python tools/ck_srcpaths.py --pe-check              # DRM/entropy verdict
    python tools/ck_srcpaths.py --pe-check --compare <other.exe>

    python tools/ck_srcpaths.py --tree                  # source modules + counts
    python tools/ck_srcpaths.py --tree --filter navmesh # matching source files

    python tools/ck_srcpaths.py --strings navmesh       # CK-only diagnostics
    python tools/ck_srcpaths.py --strings "refinaliz" --limit 100

    python tools/ck_srcpaths.py --rtti Recast           # CK-only RTTI classes
    python tools/ck_srcpaths.py --dialogs               # record editor forms
    python tools/ck_srcpaths.py --dialogs --filter Nav
...
`tools/ck_strref.py --pattern <regex>` to get the referencing code RVAs, then
`tools/skyrim_disasm.py --exe <ck> --disasm <rva>` to read the check.
