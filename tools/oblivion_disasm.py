...
Two things this does that `tools/skyrim_disasm.py` cannot (that one is x64 and
SkyrimSE-specific):
...
Usage:
    ...
    python tools/oblivion_disasm.py --fn 0x78feb0
    python tools/oblivion_disasm.py --fn 0x78feb0 --lo 0x78ff65 --hi 0x790050

    # symbolic FPU trace over a range (resolves x87 expressions)
    python tools/oblivion_disasm.py --fpu 0x78feb0 --hi 0x78ff1a

    # decode a jump table (chained sub/add dispatch is common here)
    python tools/oblivion_disasm.py --jt 0x7a7cbc --count 18 --first 6000

    # find rel32 call/jmp xrefs to an address
    python tools/oblivion_disasm.py --xref 0x7925b0

    # read a float/double constant at a VA
    python tools/oblivion_disasm.py --const 0xa3f420
"""
