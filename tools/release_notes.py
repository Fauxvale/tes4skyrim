...
PACKAGING_STEPS = ["10. Pack BSAs", "11. Pack Mod Zip"]
...
RULES: list[tuple[str, list[str]]] = [
    ...
    # Sibling-LOD conflict resolution reads the already-converted plugins and
    # rebakes only the contested tiles, so it is the global action's business
    # and not the per-plugin LOD step's.  Listed BEFORE the general LOD rule.
    (r"^asset_convert/sibling_lod\.py",        ["Merge Sibling LOD"]),
    (r"^asset_convert/(lod_gen|lod_far_gen|terrain_lod|terrain_lod_textures|"
     r"landscape_normals)\.py",                ["9. LOD", "Merge Sibling LOD"]),
    ...
    (r"^asset_convert/(bsa_pack)\.py",         ["10. Pack BSAs"]),
...
PHASE_STEPS: dict[str, list[str]] = {
    ...
    "phase_pack":               ["10. Pack BSAs"],
    "phase_pack_zip":           ["11. Pack Mod Zip"],
}
